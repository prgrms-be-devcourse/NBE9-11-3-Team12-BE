...
class NotificationIntegrationTest {
    ...
    fun async_event_listener_integration_test() {
        val event = RegistrationCompletedEvent("test@test.com", "통합테스트 마라톤", "10km")
        ...
        Mockito.verify<EmailService?>(emailService, Mockito.timeout(2000).times(1))
            .send(ArgumentMatchers.any<EmailMessage?>(EmailMessage::class.java))
    }
    ...
    fun rollback_event_listener_test() {
        val event =
            RegistrationCompletedEvent("rollback@test.com", "롤백 마라톤", "10km")
        ...
        // 한 번도 호출되지 않았음을 검증
        Mockito.verify<EmailService?>(emailService, Mockito.never())
            .send(ArgumentMatchers.any<EmailMessage?>(EmailMessage::class.java))
    }
    ...
    fun marathon_cancel_event_listener_commit_test() {
        val event = MarathonCanceledEvent(
            "서울 마라톤",
            mutableListOf<String>("user1@test.com", "user2@test.com")
        )
        ...
        Mockito.verify<EmailService?>(emailService, Mockito.timeout(2000).times(2))
            .send(ArgumentMatchers.any<EmailMessage?>(EmailMessage::class.java))
    }
